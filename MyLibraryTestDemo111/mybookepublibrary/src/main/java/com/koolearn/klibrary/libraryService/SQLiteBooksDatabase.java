package com.koolearn.klibrary.libraryService;
...
final class SQLiteBooksDatabase extends BooksDatabase {
    ...
    protected List<Bookmark> loadBookmarks(BookmarkQuery query) {
        Log.e("bookmark","bookmark::"+query.toString());
        ...
        sql.append(" bm.visible = " + (query.Visible ? 1 : 0))
        ...
        while (cursor.moveToNext()) {
            list.add(createBookmark(
                    ...
                    query.Visible,
                    (int) cursor.getLong(17),(int) cursor.getLong(18)
            ));
        }
        cursor.close();
        return list;
    ...
    protected long saveBookmark(Bookmark bookmark) {
        ...
        if (bookmarkId == -1) {
            statement = get(
                    "INSERT INTO Bookmarks (uid,version_uid,book_id,bookmark_text,original_text,creation_time,modification_time,access_time,model_id,paragraph,word,char,end_paragraph,end_word,end_character,visible,style_id,is_note) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
            );
        } else {
            statement = get(
                    "UPDATE Bookmarks SET uid=?,version_uid=?,book_id=?,bookmark_text=?,original_text=?,creation_time=?,modification_time=?,access_time=?,model_id=?,paragraph=?,word=?,char=?,end_paragraph=?,end_word=?,end_character=?,visible=?,style_id=?,is_note=? WHERE bookmark_id=?"
            );
        }

        Log.e("bookmark","bookmark::"+bookmark.getIsNote());
        synchronized (statement) {
            int fieldCount = 0;
            ...
            statement.bindLong(++fieldCount, bookmark.getIsNote());
            if (bookmarkId == -1) {
                return statement.executeInsert();
            } else {
...
//        myDatabase.execSQL("INSERT INTO BookHistory (book_id,timestamp,event) values (1,1458287221193,1)");
    ...
    {
//        this.myDatabase.execSQL("ALTER TABLE HighlightingStyle ADD COLUMN outline_color INTEGER NOT NULL DEFAULT -1");
        this.myDatabase.execSQL("ALTER TABLE Bookmarks ADD COLUMN is_note INTEGER DEFAULT 0");
        System.out.println("ALTER TABLE Bookmarks ADD COLUMN is_note INTEGER DEFAULT 0");
    }
