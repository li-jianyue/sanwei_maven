...
public final class KooReader extends KooReaderMainActivity implements ZLApplicationWindow {
    ...
    public static void openBookActivity(Context context, Book book, Bookmark bookmark,int treepostion) {
        ...
        intent.putExtra("treepostion",treepostion);
...
//    private volatile long myResumeTimestamp; // 数据同步
        ...
        Config.Instance().runOnConnect(new Runnable() {
            public void run() {
                if (treepostion==-1){
                    ...
                    myKooReaderApp.openBookByTOCPosition(myBook, bookmark,action,2);
                }
    ...
    protected void onCreate(Bundle icicle) {
        ...
        treepostion=getIntent().getIntExtra("treepostion",-1);
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            myRootView.setFitsSystemWindows(true);
            //设置可以修改状态栏背景
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            //设置状态栏为透明颜色
            getWindow().setStatusBarColor(getResources().getColor(R.color.home_title));
        }else {
            ...
            if (android.os.Build.VERSION.SDK_INT == android.os.Build.VERSION_CODES.KITKAT) {
                ...
                //设置layout不留出空间
                myRootView.setFitsSystemWindows(false);
            }
        }
...
//        myResumeTimestamp = System.currentTimeMillis();
    ...
    protected void onDestroy() {
        ((NavigationPopup) myKooReaderApp.getPopupById(NavigationPopup.ID)).removeWindow(this);
