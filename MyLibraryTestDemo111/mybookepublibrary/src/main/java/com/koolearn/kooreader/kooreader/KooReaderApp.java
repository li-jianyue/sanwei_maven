...
public final class KooReaderApp extends ZLApplication {
    ...
    public final com.koolearn.kooreader.kooreader.options.ImageOptions ImageOptions = new ImageOptions();
    public final com.koolearn.kooreader.kooreader.options.ViewOptions ViewOptions = new ViewOptions();
    public final com.koolearn.kooreader.kooreader.options.PageTurningOptions PageTurningOptions = new PageTurningOptions();
...
//						getViewWidget().repaint();
    ...
    public void openBookByTOCPosition(Book book, final Bookmark bookmark, Runnable postAction, int i) {
        if (Model != null) {
        ...
        executor.execute(new Runnable() {
            public void run() {
                openBookInternalByID(bookToOpen, bookmark, false, i);
            }
        }, postAction);
    }

    void openBookText(int treeposition) {
        final KooReaderApp kooreader = (KooReaderApp) ZLApplication.Instance();
        kooreader.addInvisibleBookmark();
        kooreader.BookTextView.gotoPosition(treeposition, 0, 0);
        kooreader.showBookTextView();
        kooreader.storePosition();
    }

    private synchronized void openBookInternalByID(final Book book, Bookmark bookmark, boolean force, int curposition) {
...
//            ZLTextHyphenator.Instance().load(book.getLanguage());
    ...
    public TOCTree getProgressTOCElement(int paramInt) {

        ArrayList localArrayList = new ArrayList();
        ZLTree.TreeIterator localTreeIterator = this.Model.TOCTree.iterator();
        Object localObject = null;
        while (localTreeIterator.hasNext()) {
            TOCTree localTOCTree = (TOCTree) localTreeIterator.next();

            TOCTree.Reference localReference = localTOCTree.getReference();
            if (localReference != null) {
                localArrayList.add(localTOCTree);
                if (localReference.ParagraphIndex >= paramInt) {
                    localObject = localTOCTree;
                    localObject = localArrayList.get(localArrayList.size() - 2);
                    break;
                }
            }
        }
        return (TOCTree) localObject;
    }

    public TOCTree getProgressTOCElementBynote(int paramInt) {
    ...
    public void addBookMark() {
        KooView localKooView = getTextView();
        ZLTextWordCursor localZLTextWordCursor = localKooView.getStartCursor();
        localKooView.clearSelection();//clearSelection
        if (localZLTextWordCursor.isNull())
            return;
        String str = localKooView.getModel().getId();
        Bookmark localBookmark = new Bookmark(Collection, Model.Book, str, new AutoTextSnippet(localZLTextWordCursor, 100), true, 1);
        Collection.saveBookmark(localBookmark);
    }
...
//            ZLTextHyphenator.Instance().load(book.getLanguage());
    ...
    public void addInvisibleBookmark(ZLTextWordCursor cursor) {
        ...
        updateInvisibleBookmarksList(new Bookmark(
                Collection, book, textModel.getId(), snippet, false, 0
        ));
    }
    ...
    public Bookmark createBookmark(int maxChars, boolean visible) {
        ...
        return new Bookmark(Collection, Model.Book, view.getModel().getId(),
                new AutoTextSnippet(cursor, maxChars), visible, 0);
    }
