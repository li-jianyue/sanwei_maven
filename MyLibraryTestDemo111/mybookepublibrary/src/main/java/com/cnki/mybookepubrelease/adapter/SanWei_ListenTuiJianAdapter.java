 ...
 * 推荐图书列表适配器
 */
public class SanWei_ListenTuiJianAdapter extends BaseAdapter {
    private Activity mContext;
    private List<SanWeiListenBookCategoryTuis> mList;
    private LayoutInflater mInflater;

    public SanWei_ListenTuiJianAdapter(Activity context) {
        mContext = context;
        mInflater = LayoutInflater.from(context);
        mList = new ArrayList<SanWeiListenBookCategoryTuis>();
    }
    public void addData(List<SanWeiListenBookCategoryTuis> list, boolean isFirstPage) {
        if (isFirstPage)
            mList.clear();
        mList.addAll(list);
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ...
        SanWeiListenBookCategoryTuis sanWeiPingLunBean = mList.get(position);
